...
def run_sync_single_company(
    ...
    for batch in range(max_batches):
        try:
            ...
            if next_ult > 0:
                ult_nsu_retornado = next_ult
            if next_max > 0:
                max_nsu_retornado = next_max

            update_cert_sync_state(clean_cnpj, str(ult_nsu_retornado), str(max_nsu_retornado), f"cStat {final_cstat}: {final_motivo}")
...
async def _background_worker_loop():
    """Loop contínuo em segundo plano que executa a sincronização para cada certificado a cada 1 hora."""
    logger.info("Iniciando Robô de Sincronização DF-e Multi-Empresa em segundo plano...")
    ...
    while True:
        try:
            ...
            await asyncio.sleep(max(60, interval_mins * 60))
