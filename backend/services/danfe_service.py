...
def _parse_resumo_doc(root, root_tag: str, ns: dict) -> Dict[str, Any]:
    """Extrai os dados disponíveis de um resNFe / resEvento para que o
    frontend consiga renderizar o que existir (cabeçalho, emitente, valor)
    e indicar que se trata apenas de um resumo sem validade fiscal.

    Esses documentos vêm da Distribuição DF-e da SEFAZ e não contêm o
    XML completo — produtos, destinatário, totais detalhados e protocolo
    não estão presentes. A UI deve mostrar isso claramente.
    """
    chNFe = (
        root.findtext(".//nfe:chNFe", default="", namespaces=ns)
        or root.findtext(".//chNFe", default="")
        or ""
    )
    cnpj_emit = (
        root.findtext(".//nfe:CNPJ", default="", namespaces=ns)
        or root.findtext(".//CNPJ", default="")
        or ""
    )
    nome_emit = (
        root.findtext(".//nfe:xNome", default="", namespaces=ns)
        or root.findtext(".//xNome", default="")
        or ""
    )
    ie_emit = (
        root.findtext(".//nfe:IE", default="", namespaces=ns)
        or root.findtext(".//IE", default="")
        or ""
    )
    valor = (
        root.findtext(".//nfe:vNF", default="0.00", namespaces=ns)
        or root.findtext(".//vNF", default="0.00")
        or "0.00"
    )
    dh_emi = (
        root.findtext(".//nfe:dhEmi", default="", namespaces=ns)
        or root.findtext(".//dhEmi", default="")
        or ""
    )
    protocolo = (
        root.findtext(".//nfe:nProt", default="", namespaces=ns)
        or root.findtext(".//nProt", default="")
        or ""
    )
    sit = (
        root.findtext(".//nfe:cSitNFe", default="", namespaces=ns)
        or root.findtext(".//cSitNFe", default="")
        or ""
    )

    tipo_label = "NF-e" if root_tag == "resNFe" else f"Evento ({root_tag})"
    return {
        "chave": chNFe,
        "protocolo": protocolo,
        "data_autorizacao": "",
        "tipo_documento": "resumo",
        "resumo_incompleto": True,
        "identificacao": {
            "numero": "",
            "serie": "",
            "data_emissao": dh_emi,
        ...
        "informacoes_adicionais": {
            "complementares": (
                f"Documento do tipo {root_tag} (resumo da Distribuição DF-e). "
                "Não contém produtos, destinatário nem protocolo completo. "
                "Para visualizar o DANFE completo, importe o arquivo XML "
                "completo (nfeProc) ou consulte a NF-e diretamente na SEFAZ "
                "com seu certificado digital."
            ),
        },
        "situacao_resumo": sit,
        "tipo_documento_resumo": tipo_label,
    }
...
def parse_nfe_xml(xml_bytes: bytes) -> Dict[str, Any]:
    ...
    # Detecta o tipo de documento pela tag raiz. NF-e completa (nfeProc/NFe)
    # é renderizada normalmente; resNFe (resumo da Distribuição DF-e) e
    # resEvento (resumo de evento) não têm produtos/destinatário e geram
    # DANFE incompleto — sinalizamos para o caller decidir o que fazer.
    root_tag = etree.QName(root.tag).localname
    if root_tag in ("resNFe", "resEvento"):
        return _parse_resumo_doc(root, root_tag, ns)
    ...
    if transp is not None:
        transportadora["modalidade_frete"] = transp.findtext("nfe:modFrete", default="9", namespaces=ns)
        ...
        if transport_node is not None:
            transportadora = {
                ...
                "modalidade_frete": transportadora.get("modalidade_frete", "9"),
            }
...
def build_synthetic_nfe_xml(doc: Dict[str, Any]) -> bytes:
    ...
    if not produtos:
        ...
        emitente = doc.get("emitente_nome") or ""
        produtos = [{
            "n_item": 1,
            "codigo": "001",
            "descricao": f"MERCADORIA - {emitente}" if emitente else "MERCADORIA / PRODUTO VENDIDO",
            "ncm": "85171300",
            "cfop": "5102",
            "unidade": "UN",
            "quantidade": 1.0,
    ...
    for idx, p in enumerate(produtos, start=1):
        ...
        if not descricao or descricao in ("PRODUTO", "PRODUTO / MERCADORIA VENDIDA"):
            emitente = doc.get("emitente_nome") or ""
            descricao = f"MERCADORIA - {emitente}" if emitente else f"PRODUTO {idx:03d}"
        ...
        etree.SubElement(prod, f"{{{ns}}}xProd").text = descricao
